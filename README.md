...
![Version](https://img.shields.io/badge/version-0.133b-purple)
...
VERSION=0.133b BUILD_NUMBER=135 ARCH=arm64 ./Scripts/build-release.sh

# Intel
VERSION=0.133b BUILD_NUMBER=135 ARCH=x86_64 ./Scripts/build-release.sh
```
...
The **Discover** scope provides the native Assembly64 chart feeds—**Demos** (Demo Top 200), **Games**, **Graphics**, **Music**, **OneFile Demos**, and **Tools**—plus recent releases. The chart lists use Assembly64’s dedicated `/leet/charts/{type}` endpoint, so their ranks are native rather than Stream64-owned popularity scores; recent releases use the same AQL sort/filter protocol as normal search. Categories/presets cache for 24 hours and chart/query pages cache briefly so switching lists is responsive; binary downloads are never cached. The browser enforces a content-derived minimum height so the metadata thumbnail cannot be clipped at the smallest window size.
